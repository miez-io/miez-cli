...
bundle that miez can build, load, validate, and bootstrap. It exists so team
authors can keep configuration beside the Markdown artifact it describes while
operators receive one deterministic team index for installation and CLI use.
...
- **In scope:** the package metadata manifest, Markdown frontmatter, generated
  team index, worker/skill/workflow artifacts, validation, and starter-team
  authoring.
- **Out of scope:** provider-specific rendering, remote download mechanics, and
  execution of workers or workflows.
...
that identifies the team and declares its workers, skills, workflows, model
catalog, and MCP declarations.

Acceptance:

- WHEN a compatible generated team manifest is loaded THEN its declared
  workers, skills, and workflows are available for validation, completion, and
  rendering.
...
- WHEN build inputs are valid THEN the generated `miez.generated.yaml` contains the team
  metadata, worker paths and configuration, available skills, workflow paths
  and configuration, model catalog, and MCP declarations.
...
When a team is built, the system shall require each worker, skill, and workflow
Markdown artifact to have valid frontmatter and a path contained within the
team source. When a generated team is installed, the system shall validate the
declared artifact paths without reconstructing configuration from frontmatter.
...
When a team artifact is built, the system shall derive each artifact's CLI
configuration from its frontmatter without requiring a duplicate manual entry
for that artifact in `miez.generated.yaml`.
...
metadata, generated index, identifiers, worker/skill/workflow references,
models, MCP relationships, and referenced files.
...
package metadata manifest and three artifact directories containing one worker,
one skill, and one workflow template without overwriting an existing team.

Acceptance:

- WHEN the destination does not contain the team THEN `miez.yaml`,
  `workflows/`, `workers/`, and `skills/` with one frontmatter template each
  are created in a flat team directory, and no generated `miez.generated.yaml` is
...
- WHEN a team author reads the guide THEN it explains the CLI lifecycle,
  available worker kinds, required frontmatter, skill assignment, workflow
  membership, and the distinction between authored files and the generated
  team index.
...
- WHEN a package containing the support bundle is built THEN the generated
  team index contains no worker, skill, or workflow entry derived only from a
  support file.
...
team index that can be installed without parsing authoring frontmatter.

Acceptance:

- WHEN build succeeds THEN `miez.generated.yaml` is present beside `miez.yaml` and its
  references resolve to files in the package.
