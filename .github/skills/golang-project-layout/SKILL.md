...
- [ ] For deployable services: add `internal/meta/` (version injection), multi-stage `Dockerfile`, and `.github/workflows/` CI — see `golang-cloud-native-service` skill

## Related Skills

→ See `samber/cc-skills-golang@golang-cli` skill for CLI tool structure and Cobra/Viper patterns. → See `samber/cc-skills-golang@golang-dependency-injection` skill for DI approach comparison and wiring. → See `samber/cc-skills-golang@golang-lint` skill for golangci-lint configuration. → See `golang-cloud-native-service` skill for Dockerfile, GitHub Actions build+push, version ldflags, and OpenTelemetry scaffolding. → See `samber/cc-skills-golang@golang-design-patterns` skill for architectural patterns.
